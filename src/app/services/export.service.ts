...
export class ExportService {
  ...
  startLiveRecording(mimeType: string = 'audio/webm;codecs=opus') {
    ...
    const recorder = new MediaRecorder(dest.stream, { mimeType: supportedType, audioBitsPerSecond: 256000 });
    const chunks: Blob[] = [];
  ...
  public audioBufferToWav(buffer: AudioBuffer): ArrayBuffer {
    ...
    view.setUint32(offset, 36 + buffer.length * numOfChan * 4, true); offset += 4;
    ...
    view.setUint32(offset, sampleRate * numOfChan * 4, true); offset += 4;
    view.setUint16(offset, numOfChan * 4, true); offset += 2;
    view.setUint16(offset, 32, true); offset += 2;
    writeString(view, offset, 'data'); offset += 4;
    view.setUint32(offset, buffer.length * numOfChan * 4, true); offset += 4;
    for (let i = 0; i < buffer.length; i++) {
      for (let channel = 0; channel < numOfChan; channel++) {
        view.setFloat32(offset, buffer.getChannelData(channel)[i], true); offset += 4;
      }
    }
    return bufferOut;
  }
