...
describe('AuthService', () => {
  ...
  it('registers profiles against the created user id', fakeAsync(() => {
    ...
    const req = httpMock.expectOne(r => r.url.includes('/auth/session'));
  ...
  it('normalizes email casing and spacing during login', fakeAsync(() => {
    ...
    tick(1000);
    let req = httpMock.expectOne(r => r.url.includes('/auth/session'));
    req.flush({ token: 'mock-jwt-token' });
    tick();
    ...
    tick(1000);
    req = httpMock.expectOne(r => r.url.includes('/auth/session'));
  ...
  it('does not send a confirmation email when login fails', fakeAsync(() => {
    ...
    tick(1000);
    httpMock.expectNone(r => r.url.includes('/auth/session') && r.method === 'POST');
