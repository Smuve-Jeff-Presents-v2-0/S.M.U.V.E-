...
export class ThaSpotComponent implements OnInit, OnDestroy {
  ...
  confirmLaunch() {
    ...
    this.matchmakingTimerId = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        this.profileService.recordGameLaunch(game.id);
      }
    }, 150);
  }
  ...
  private findMasteredRoom(stats: Record<string, any> = {}) {
    ...
    const topRoomId = Array.from(roomScores.entries()).sort((a, b) => b[1] - a[1])[0]?.[0];
  ...
  private scoreRecommendation(game: Game, rail: RecommendationRail, stats: Record<string, any>, roomId: string) {
  ...
  private getTotalPlays() {
    return Object.values(this.profileService.profile().gameStats || {}).reduce((total, stat: any) => total + (stat?.plays || 0), 0);
  }
  ...
  toggleQuickFilter(f: QuickFilter) { this.quickFilters.update(fs => fs.includes(f) ? fs.filter(e => e !== f) : [...fs, f]); }
}
